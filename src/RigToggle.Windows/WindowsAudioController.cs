...
/// mutation. SetDefault applies the requested device to all three Windows audio
...
/// eCommunications) independently (D-02) so each role's capture is exact and
/// self-contained — a failed read for one role falls back to a null AudioRoleState
...
public sealed class WindowsAudioController : IAudioController
{
    ...
    // for every SetDefault cycle. Values are numerically identical (0/1/2) but
    ...
    // Shared by SetDefault (D-01/D-02/D-03/D-04): creates a fresh
