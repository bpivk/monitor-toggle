...
**Domain:** Windows-only desktop GUI utility (display/audio/process control automation), extended with tray/hotkey/CLI/IPC/multi-monitor automation, extended with system-theme-aware visual polish + tray icon redesign, now extended again with exe-size reduction (non-trimming levers), a live manual per-monitor toggle panel, and optional/nullable settings + explicit per-mode monitor configuration
**Researched:** 2026-07-24 (v1.0 foundation) — updated 2026-07-26 (v1.1 additions) — updated 2026-08-02 (v1.2 additions) — updated 2026-08-04 (v2.0 additions)
**Confidence:** HIGH (v1.0 stack, shipped and validated) / MEDIUM-HIGH (v1.1 additions, shipped and validated) / HIGH (v1.2 theming API contract, MEDIUM for a few runtime-behavior caveats) / HIGH (v2.0 additions — MSBuild-property and dependency-hygiene claims verified directly against official docs, package contents, and this repo's own source; MEDIUM for exact compression/satellite-language MB-savings figures, which are workload-dependent and not yet empirically measured against this specific exe)

---

# v2.0 Milestone Additions (2026-08-04)

Scope: reduce the self-contained exe's size **without** enabling IL trimming (`PublishTrimmed` stays a hard project constraint per CLAUDE.md/PROJECT.md); add a live, on-demand manual per-monitor enable/disable GUI panel with icon-based status; make the companion-app launch target and per-role audio devices optional (nullable); replace Normal mode's snapshot-restore with an explicit, symmetric-with-Rig-mode per-mode monitor target-state configuration. This section covers **only** what's new for v2.0 — the v1.0/v1.1/v1.2 stack below is already shipped and validated; it is not re-researched here.

## Headline Finding

**No new NuGet packages are required for any of the three v2.0 stack questions.** Exe-size reduction is 100% MSBuild-property configuration plus trimming your own explicit `PackageReference`s down to what's actually used (not IL trimming) — zero new libraries. The manual monitor panel is best served by a small hand-rolled tile `UserControl` (icon + label, click-to-toggle) inside a `FlowLayoutPanel`, reusing this project's own already-validated GDI+ icon-drawing approach (`RigToggle.IconGen`) rather than reaching for WinForms' built-in `ListView.Tile` view or a third-party component suite. The optional/nullable settings and per-mode monitor config work needs **literally nothing new**: `AppSettings.cs` is already a 100%-nullable-by-design model with a documented "null means never configured" convention, `System.Text.Json` (already the only serializer in use) already round-trips missing/null fields correctly, and this exact mechanism has already shipped once for the v1.0→v1.1 migration — v2.0 just adds more nullable properties and, if needed, one more null-check migration guard following the same pattern.

## Recommended Stack Additions

### Exe Size Reduction (MSBuild properties only — zero new packages)

| Change | Where | Effect | Why Recommended |
|--------|-------|--------|------------------|
| `EnableCompressionInSingleFile=true` | `win-x64.pubxml` | Compresses the embedded managed assemblies inside the single-file bundle | Official, documented (`learn.microsoft.com/dotnet/core/deploying/single-file/overview`), and explicitly listed as independent of `PublishTrimmed` — Microsoft's own docs describe it as *"can be combined with"* trimming, not requiring it. Real size reduction, but Microsoft's docs are explicit about a startup-cost tradeoff (assemblies must decompress into memory at launch) and say to *measure* both size and startup impact rather than assume — budget a rig-side before/after cold-start timing check alongside the file-size check, not size alone. |
| `SatelliteResourceLanguages=en` | `RigToggle.App.csproj` root `PropertyGroup` | Drops non-English localized satellite resource assemblies pulled in by the BCL/WinForms/any referenced package | Independent of trimming; a real, specifically WinForms-relevant win (WinForms ships localized resources for many cultures your English-only, single-user app never uses). Must be applied at the project that references the package with the resources — since this is a solution-wide English-only app, setting it in `RigToggle.App.csproj` (the publish entry point) is sufficient. |
| `InvariantGlobalization=true` | `RigToggle.App.csproj` | Removes ICU globalization data from the self-contained payload | The single largest lever available here — ICU data is tens of MB in a self-contained Windows publish. Verified this app has **zero** `CultureInfo`/culture-sensitive formatting calls anywhere in `src/` (grep-confirmed directly against the codebase, not assumed) — invariant-culture behavior (fixed, OS-locale-independent formatting/casing/sorting) changes nothing this app currently relies on. Flag for future maintainers: re-grep for `CultureInfo`/culture-sensitive `ToString` before assuming this stays safe if a future feature adds date/number formatting. |
| Replace the `NAudio` meta-package with `NAudio.Core` + `NAudio.Wasapi` directly | `RigToggle.Windows.csproj` | Drops `NAudio.Asio`, `NAudio.Midi`, `NAudio.WinMM`, `NAudio.WinForms` and their own transitive dependencies (`Microsoft.Win32.Registry` 4.7.0, `System.Resources.Extensions`) that the meta-package pulls in but this project never uses | Grep-confirmed (`grep -rn "using NAudio" src/`): the codebase only uses `NAudio.CoreAudioApi` (`MMDeviceEnumerator`/`MMDevice`), which lives in the `NAudio.Wasapi` package (itself depending only on `NAudio.Core`). NAudio 2.x is deliberately split into multiple NuGet packages precisely so consumers can avoid ASIO/MIDI/WinMM/WinForms-control weight they don't need — referencing the two needed packages directly is a supported, zero-code-change swap (same namespaces, same types, same version 2.3.0). Modest in absolute terms (~150–300 KB of assemblies plus one avoided transitive package, measured directly from the published `.nupkg` contents) but genuinely free — real dead weight removed with zero downside. |
| **Do not** add `PublishReadyToRun=true` as a size lever | — | R2R precompiles native code *alongside* the IL (both ship), which **increases** single-file size | Confirmed directly against Microsoft's ReadyToRun deployment docs: R2R binaries are larger than pure-IL equivalents by design. R2R is a startup-latency lever, not a size lever — it's the opposite of this milestone's stated goal. No startup-latency complaint has been raised for this app, so there's no offsetting reason to add it here either. |
| `<DebugType>embedded</DebugType>` (optional, low priority) | `RigToggle.App.csproj` | Folds the PDB into the DLL instead of a separate `.pdb` file | Only matters if PDBs currently ship alongside the distributed `.exe` (check `release.yml`'s upload step). If they aren't already attached to GitHub Releases, this only tidies local build output — not the distributed artifact. |

### GUI: Live Manual Monitor Panel

| Technology | Version/Source | Purpose | Why Recommended |
|------------|-----------------|---------|-------------------|
| Hand-rolled tile `UserControl` (a `PictureBox` status icon + `Label` + click handler) hosted in a `FlowLayoutPanel` | Built into WinForms — one new small class, no package | Per-monitor status tile: icon reflecting enabled/disabled state, friendly-name label, click-anywhere-on-tile-to-toggle | Matches this project's own established, rig-proven pattern from Phase 12/13: every visually important surface in this app has ended up hand-drawn/hand-themed (explicit-color `FlatStyle.Flat` buttons with hover/pressed colors set explicitly, `Panel`+`Label` replacing `GroupBox`, procedurally-drawn `.ico` glyphs) specifically *because* stock WinForms control theming/rendering repeatedly under-delivered on real rig verification (the `FlatStyle.Flat` dark-mode bug, `GroupBox`'s unthemable 3D bevel — both documented in this file's v1.2 section). A `FlowLayoutPanel` of small owner-built tiles gives full control over both the icon (reuse the same GDI+ drawing code already proven in `RigToggle.IconGen`, extended with 2 new small glyph variants — filled/enabled vs. outlined-or-greyed/disabled monitor silhouette) and the color palette (same explicit-color theming already used everywhere else), and naturally re-flows for however many monitors are actually connected rather than assuming a fixed count. |
| Extend `RigToggle.IconGen`'s existing GDI+ drawing approach with 2 new small status glyphs | Existing dev-time tool, extended (not a new dependency) | Renders the per-tile enabled/disabled status icon | Zero new asset pipeline, zero new dependency, zero new design tool — this is exactly the tool the project already built and rig-validated (Phase 13, including a pixel-level seam diagnostic) for procedural icon generation. Extending it with 2 more small glyph variants is meaningfully lower risk than sourcing new artwork, an icon font, or a new drawing library. |

### Alternatives evaluated and explicitly not recommended

| Recommended | Alternative | Why not for this project |
|-------------|-------------|---------------------------|
| Hand-rolled tile `UserControl` in `FlowLayoutPanel` | `ListView` in `View.Tile` mode (built-in WinForms, zero extra code) | Functionally close to the ask — icon + label + subitems per tile, built into the framework with a documented `TileSize`/`Columns` API. Two reasons it's not the primary recommendation here: (1) `ListView` is a Win32-common-control composition, and .NET 10's own dark-mode docs call out exactly this class of control (composing native Win32 common controls, e.g. embedded scrollbars) as needing the manual `ControlStyles.ApplyThemingImplicitly` opt-in for correct theming — an interaction this project has never used or rig-tested, on a control this app has never referenced before; (2) `ListView`'s selection/`ItemActivate`/checkbox conventions are a worse fit for "click the tile anywhere, it toggles immediately" than a custom control's own `Click` handler. Worth reconsidering only if the hand-rolled tile proves to be more code/maintenance than expected — it remains a legitimate, zero-dependency fallback. |
| Hand-rolled tile `UserControl` | Third-party WinForms component suites (Krypton Toolkit tile/dashboard controls, Infragistics `TilePanel`) | Same reasoning already recorded in this file's v1.2 section, where Krypton was evaluated and rejected as a *primary* theming mechanism: adds real dependency surface and a second theming API to keep synced with `SystemColorMode`, for a UI element (2–4 status tiles for a small personal multi-monitor rig) simple enough to hand-roll in well under 100 lines. Infragistics specifically is also a commercial/licensed component, which this project has no other dependency of that class. |

## Optional/Nullable Settings & Per-Mode Monitor Config — No Stack Change Needed

Confirmed by reading `src/RigToggle.Core/Models/AppSettings.cs`, `src/RigToggle.Core/Persistence/JsonSettingsStore.cs`, and every `.csproj`/`.pubxml` in the solution directly:

- `<Nullable>enable</Nullable>` is already set in every project (`App`, `Core`, `Windows`, both test projects). v2.0's new optional fields — a nullable companion-app launch path (already `CompanionAppPath` is `string?`), nullable per-role audio device IDs (already `string?` for `NormalAudioDeviceId`/`RigAudioDeviceId`), and a new explicit Normal-mode monitor target-state pair mirroring the existing `MonitorsToDisable`/`MonitorsToEnable` Rig-mode fields — are just more `string?`/`List<string>?` properties on a model whose own class doc already states the project-wide convention: *"All fields nullable: a null field means 'never configured' (first run), not 'stale'."* Nothing about "optional app-launch-target" or "optional per-role audio" requires a schema/serialization capability this app doesn't already have; it requires application-logic changes in the toggle orchestrator to treat "field is null" as "skip this step," which is a `RigToggle.Core` logic change, not a stack/library change.
- `System.Text.Json` (BCL, already the project's only serializer) already round-trips missing/null properties correctly, and this exact pattern has already shipped once: `JsonSettingsStore.Load()`'s existing v1.0→v1.1 migration guard (`MonitorDevicePath` present AND `MonitorsToDisable is null` ⇒ synthesize the plural field) is the same "detect legacy/absent field, migrate once, never re-trigger" pattern v2.0's Normal-mode monitor-config addition needs if its on-disk shape changes meaningfully. That guard was itself hardened once already (CR-01: check `is null` only, never null-or-empty, so a user's deliberate empty set isn't re-corrupted) — follow that already-proven exact pattern for any new v2.0 migration guard rather than inventing a new one.
- No `JsonSerializerOptions` changes are strictly required. If the on-disk JSON should omit unset optional fields for readability (the settings file is meant to be user-hand-editable, per the original v1.0 stack rationale for choosing JSON over the registry), add `DefaultIgnoreCondition = JsonIgnoreCondition.WhenWritingNull` to the existing static `Options` instance in `JsonSettingsStore` — purely cosmetic, zero behavior risk, entirely optional.

## What NOT to Use (v2.0 additions)

| Avoid | Why | Use Instead |
|-------|-----|--------------|
| `PublishTrimmed=true` (still, unchanged) | Unchanged hard constraint from CLAUDE.md/PROJECT.md/`win-x64.pubxml`'s own comment — COM-interop (`IPolicyConfig`) and P/Invoke marshalling (`WindowsDisplayAPI`) code paths are exactly what IL trimming's reachability analysis misidentifies as dead code and strips, breaking those calls only at runtime | The MSBuild-property levers above (compression, satellite-language stripping, invariant globalization, dependency hygiene) — all real size reduction with zero linker risk |
| `PublishReadyToRun=true` used as a "size" lever | Increases single-file size (ships native-precompiled code *in addition to* IL, not instead of it) — solves a startup-latency problem this milestone never raised, at the direct cost of the metric it's trying to reduce | Leave R2R off. If startup latency ever becomes an explicit complaint, evaluate it then as its own separate trade-off, not folded into a size-reduction pass. |
| Third-party WinForms tile/dashboard component libraries (Krypton, Infragistics, MetroFramework) for the monitor panel | Same reasoning as this file's v1.2 tray-icon/theming section: adds dependency surface and a second theming system to keep synced with `SystemColorMode`, for a UI element (2–4 status tiles) simple enough to hand-roll | Hand-rolled `UserControl` tile in a `FlowLayoutPanel`, reusing `RigToggle.IconGen`'s existing GDI+ drawing code for the status glyphs |
| Switching off `System.Text.Json` (e.g. to a different serializer) "to support optional fields" | Optional/nullable fields are not a serializer capability gap — `System.Text.Json` plus nullable reference types already fully cover this, and the project has already shipped one real schema migration (v1.0→v1.1) on exactly this mechanism | Keep `System.Text.Json`; add more nullable properties on `AppSettings` and, if the on-disk shape changes meaningfully, one more null-check-only migration guard in `JsonSettingsStore.Load()` following the CR-01-hardened pattern already there |
| Keeping the umbrella `NAudio` meta-package "because it's already referenced" | It transitively pulls `NAudio.Asio`/`NAudio.Midi`/`NAudio.WinMM`/`NAudio.WinForms` and their own dependencies, none of which this project's grep-confirmed `NAudio.CoreAudioApi`-only usage needs | `NAudio.Core` + `NAudio.Wasapi` 2.3.0 direct references — same types, same version, no code change, less dead weight |

## Version Compatibility (v2.0 additions)

| Property/Package | Compatible With | Notes |
|-------------------|-------------------|-------|
| `EnableCompressionInSingleFile`, `SatelliteResourceLanguages`, `InvariantGlobalization`, `IncludeNativeLibrariesForSelfExtract` (already set) | .NET 10, win-x64 self-contained single-file (already this project's exact publish profile) | All stable, non-experimental MSBuild properties, unchanged in mechanism since the .NET 6/7 era; no .NET-10-specific caveats found. |
| `NAudio.Core` + `NAudio.Wasapi` 2.3.0 | Identical version already referenced transitively today via the `NAudio` meta-package | No version bump, no API surface change — `NAudio.CoreAudioApi.MMDeviceEnumerator`/`MMDevice` (the only types this project uses) live in `NAudio.Wasapi` 2.3.0 exactly as they do today via the umbrella package; this is a `PackageReference` swap only. |
| Hand-rolled tile `UserControl`/`PictureBox`/`FlowLayoutPanel` under `Application.SetColorMode` dark mode | .NET 10 WinForms (already in use) | `FlowLayoutPanel` (a `Panel` derivative) and `PictureBox`/`Label` are already used elsewhere in this app and already correctly dark-themed (Phase 12) — the new tile control should follow the same explicit-color theming pattern already established in `MainForm`/`SettingsForm`, not a new one. |

## Sources (v2.0 additions)

- https://learn.microsoft.com/en-us/dotnet/core/deploying/single-file/overview — fetched directly 2026-08-04: confirmed `EnableCompressionInSingleFile` is documented as combinable with (not requiring) trimming, its explicit startup-cost tradeoff warning ("measure both the size change and startup cost"), `IncludeNativeLibrariesForSelfExtract` semantics, and that R2R/single-file/trim are independently combinable options — HIGH confidence
- https://learn.microsoft.com/en-us/dotnet/core/deploying/ready-to-run — confirmed R2R binaries are larger than pure-IL equivalents (native code ships alongside IL, not instead of it) — HIGH confidence
- https://andrewlock.net/disabling-localized-satellite-assemblies-during-dotnet-publish/ — fetched directly: confirmed `SatelliteResourceLanguages` syntax/scoping (must be set on the project referencing the package with resources), its independence from trimming, and a cited ~15 MB WinForms-app-specific size-reduction figure — MEDIUM-HIGH confidence (single blog source for the exact MB figure; the mechanism itself is corroborated by Microsoft's own MSBuild-properties reference) 
- https://github.com/dotnet/runtime/blob/main/docs/design/features/globalization-invariant-mode.md and https://learn.microsoft.com/en-us/dotnet/core/extensions/globalization-icu — confirmed `InvariantGlobalization` removes the ICU dependency/data and its behavior-change scope (culture-invariant formatting/casing/sorting) — HIGH confidence
- Direct grep of this repo (`grep -rn "CultureInfo" src/`, `grep -rln "ToString(\""`) — confirmed zero culture-sensitive API usage exists in this codebase today; used as the basis for assessing `InvariantGlobalization` as low-risk specifically for this project, not as a general claim — HIGH confidence (verified against actual project source)
- https://api.nuget.org (nuspec + `.nupkg` contents fetched and extracted directly for `NAudio`/`NAudio.Core`/`NAudio.Wasapi`/`NAudio.Asio`/`NAudio.Midi`/`NAudio.WinMM`/`NAudio.WinForms` 2.3.0) — confirmed the meta-package's transitive dependency graph (including `NAudio.Asio`/`NAudio.WinMM` both pulling `Microsoft.Win32.Registry` 4.7.0, `NAudio.WinForms` pulling `System.Resources.Extensions`) and each sub-package's actual assembly size (`NAudio.Asio.dll` ~34 KB, `NAudio.Midi.dll` ~46 KB, `NAudio.WinMM.dll` ~58 KB, `NAudio.WinForms.dll` ~34 KB) — HIGH confidence, verified directly against package contents
- GitHub source browse (`naudio/NAudio` `release/2.x` branch, `NAudio.Wasapi/CoreAudioApi/MMDeviceEnumerator.cs`) — confirmed `MMDeviceEnumerator`/`MMDevice` live in the `NAudio.Wasapi` package's `NAudio.CoreAudioApi` namespace, not a separate "Core" package — HIGH confidence
- Direct grep of this repo (`grep -rn "using NAudio" src/`) — confirmed this project only uses the `NAudio.CoreAudioApi` namespace, nothing from the Asio/Midi/WinMM/WinForms sub-packages — HIGH confidence
- https://learn.microsoft.com/en-us/dotnet/desktop/winforms/whats-new/net100 — fetched directly: confirmed dark mode is GA/non-experimental in .NET 10, and the `ControlStyles.ApplyThemingImplicitly`/`CreateParams` caveat for controls composing native Win32 common controls — the basis for flagging `ListView` as a theming-risk-relative-to-hand-rolled option rather than a confirmed-broken one — HIGH confidence
- This file's own v1.2 section (already-validated prior finding) — stock WinForms control theming repeatedly under-delivered on rig verification in this exact project (`FlatStyle.Flat` dark-mode bug, `GroupBox`'s unthemable bevel), and the project's established response was hand-rolled explicit-color theming — used as the direct basis for recommending the same pattern for the new monitor-panel tiles instead of `ListView.Tile` or a third-party component
- `src/RigToggle.Core/Models/AppSettings.cs`, `src/RigToggle.Core/Persistence/JsonSettingsStore.cs`, all `.csproj`/`.pubxml` files (read directly) — confirmed the current nullable/serialization/publish-property baseline, establishing that zero stack changes are needed for the optional-fields and per-mode-config parts of this milestone's scope
- `.planning/PROJECT.md` — v2.0 milestone framing (target features, constraints, "no trimming" hard constraint, existing validated capabilities) used as ground truth for scope boundaries
...
| NAudio | 2.3.0 | Enumerate audio render endpoints (friendly names, device IDs, default device query) via `MMDeviceEnumerator` | Use for everything *except* the actual "set default" call: listing playback devices for the settings dropdown, resolving a saved device ID back to a live device at toggle-time, and reading the *current* default device before switching (for state restore). Actively maintained (v2.3.0, updated March 2026), MIT licensed, the de facto standard .NET audio library. Its `MMDeviceEnumerator`/`MMDevice` wrap the same public `IMMDeviceEnumerator` Core Audio API that Windows itself uses for enumeration — fully documented and stable, unlike the "set default" call. **v2.0 update: reference `NAudio.Core` + `NAudio.Wasapi` directly instead of the `NAudio` meta-package — see the v2.0 section above.** |
...
**Recommended `.csproj` publish settings (updated for v2.0 — see the dedicated section above for the size-reduction properties):**
...
dotnet add package NAudio.Core --version 2.3.0
dotnet add package NAudio.Wasapi --version 2.3.0
...
| NAudio 2.3.0 | .NET 10 | Actively maintained against current .NET; no compatibility concerns. See v2.0 section above for the recommended `NAudio.Core`+`NAudio.Wasapi` split (same version, less transitive weight). |
...
---
*Stack research for: Windows desktop GUI utility (display/audio/process automation) — v1.0 foundation + v1.1 tray/hotkey/CLI/IPC/multi-monitor additions + v1.2 visual polish (theming + tray icon redesign) additions + v2.0 exe-size-reduction/manual-monitor-panel/optional-settings additions*
*Researched: 2026-07-24 (v1.0) — updated 2026-07-26 (v1.1) — updated 2026-08-02 (v1.2) — updated 2026-08-04 (v2.0)*
