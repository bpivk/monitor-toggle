...
1. **Does `Application.Exit()` cleanly terminate the message loop when the main form was started via `ApplicationContext` and never shown?**
...
2. **Should `WindowsAutostartConfigurator` get a dedicated `RigToggle.Windows.Tests` test, given it requires a real registry write?**
