...
**Shipped: v2.1 Modern UI Redesign & Theme Backlog (2026-08-17)**

v2.1 replaced both windows' bolted-on-feature layout with an intentional, modern design and closed out the deferred theme backlog. MainForm became a monitor-tile dashboard (one tile per monitor, icon+number+live status, Identify, keyboard nav) that absorbed the standalone Manual Monitor Panel's capability and retired it outright — deleted, not deprecated. The Rig/Normal toggle became a custom-drawn, accent-tinted `ToggleSwitch` control. Interactive elements now read the live Windows accent color instead of a fixed palette. SettingsForm was fully migrated from hardcoded `Panel`/pixel positioning to a `TableLayoutPanel`-based layout (2 gap-closure rounds after the first rig pass failed outright, plus a post-rig code-review fix round for a working-area clamp bug). A manual System/Light/Dark theme override was added, collapsing three independently-mirrored `IsDark`/`IsDarkTheme` resolution points into one shared `OverridableThemeProvider` decorator — the structural fix for a consistency risk the milestone's own research had flagged in advance. All five phases were rig-verified on real Windows 11 hardware; the milestone integration audit found 14/14 requirements satisfied, 0 cross-phase gaps, and no regression to the untouched core toggle logic. 10 non-blocking tech-debt items tracked, none blocking (see Context below).
